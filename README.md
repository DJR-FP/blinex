...
[![Version](https://img.shields.io/badge/version-v0.22.0-blue)](#roadmap)
...
│  Subnet / exit node routing  (iptables MASQUERADE / New-NetNat)      │
...
3. In parallel, the agent runs **ICE** (STUN + the TURN relay) to try for a
   **direct** peer-to-peer path. A full-size (1200-byte) probe runs over the
   candidate ICE connection, and the agent switches that peer's send path from
   relay to direct only after **three consecutive** probes come back answered —
   a marginal NAT binding that passes small packets but drops real traffic
   never clears that bar. A single missed probe reverts to relay and holds it
   there with exponential backoff (15s → 5min, forgiven after two minutes of
   stability), so a flapping path degrades to "always relay" rather than
   oscillating.
4. On kernel-TUN devices the agent installs a `100.64.0.0/10` route so mesh
   traffic reaches the WireGuard interface — a `blinex0` device via `netlink` on
   Linux, a wintun adapter via PowerShell on Windows — and sets up subnet
   routing / exit-node NAT with iptables MASQUERADE (Linux) or `New-NetNat` +
   `New-NetRoute` (Windows). In netstack mode (no `/dev/net/tun`, e.g. an
   unprivileged LXC) it serves inbound traffic in userspace **and can still act
   as a subnet router or exit node**: a gVisor netstack forwarder proxies
   mesh→LAN (or mesh→internet for a `0.0.0.0/0` exit) TCP/UDP/ICMP out through
   the host socket (auto-SNAT, no iptables) — the same userspace approach
   NetBird/Tailscale use to route through containers.
5. **ACL enforcement runs in all three modes**, with the same first-match-wins,
   default-deny semantics in each: an ordered `BLINEX-ACL` iptables chain on
   Linux kernel-TUN peers; an in-process filter on every decrypted packet
   entering the OS on Windows kernel-TUN peers (Windows Firewall can't express
   default-deny-plus-allows — its block rules always beat allow rules); and the
   netstack forwarder's own check on everything it routes.
...
### Done ✅
- [x] **Exit node: per-device opt-in** — using an exit node is now a choice each device makes, not a mesh-wide mode. Advertising `0.0.0.0/0` *offers* an exit node; it no longer imposes one. Previously the agent scanned the route list for any enabled default route belonging to another peer, so a single advertisement redirected the default route of **every** peer on the account at once — no opt-in, no way to try an exit node without disrupting everyone, and nothing resembling Tailscale's or NetBird's per-device selection. Selection now lives on the peer (`PUT /peers/:key/exit-node`, empty to clear) and is resolved and validated server-side, then delivered to that one agent in `NetworkConfig.exit_node`. Validation matters more than it looks: a selection whose gateway has vanished, stopped advertising `0.0.0.0/0`, or is the peer itself resolves to *no exit node*, because sending a device's entire default route to a gateway that isn't offering one is a worse failure than not using an exit node at all. Withdrawing the advertisement withdraws it from everyone using it, with no per-device change needed. (v0.22.0)
- [x] **Fixed: Windows subnet routing forwarded nothing, and said it was fine** — the first live exercise of v0.21.0's `routing_windows.go` (Win10 Pro 22H2, wintun kernel-TUN, advertising a LAN to a Linux consumer). The consumer installed the route correctly; the gateway failed in four ways, every one of them silent. `EnableForwarding()` was literally `return nil` — a stub, on the platform where it mattered — because the code assumed WinNAT "does forwarding and NAT together"; it does not, NAT rewrites addresses but does not make a host a router, and live every interface still read `Forwarding: Disabled`. `New-NetNat` could not run at all: `MSFT_NetNat` is not registered on a stock Win10 without Hyper-V or Containers, so every call failed with CIM "Invalid class" (HRESULT 0x80041010), a message naming no remedy. The engine then logged `advertising routes — forwarding enabled` unconditionally, *after both of those had failed*, so a gateway that could not forward a single packet reported itself healthy. And the ACL filter dropped every forwarded packet — the subtle one, because the error is in a comparison rather than in code: `aclfilter.go` was written to mirror the netstack filter and polices every decrypted packet, but on Linux it is not `BLINEX-ACL` that governs transit at all — `AddMasquerade` *inserts* `iptables -I FORWARD -i blinex0 -j ACCEPT` above the jump to the chain, so Linux accepts forwarded traffic before the ACL ever sees it. Windows policed it, rules expand to mesh peer IPs, a LAN destination matched nothing, and default-deny ate it. Now: forwarding is really enabled per-interface, missing WinNAT produces an error naming the optional feature to enable (or the static route that removes the need for NAT), the log tells the truth and uses ERR for each broken combination, and only packets addressed to this host are policed — with an unknown local-address set failing closed. Verified live with `pktmon` on the gateway: the echo request now leaves on the physical NIC as `100.64.0.9 > 192.168.100.1`, where before it never left the box. It is still un-NATted, so replies need WinNAT or a LAN static route — reported now rather than hidden, and tracked in docs/TESTING.md §7c. (v0.21.3)
- [x] **Fixed: Magic DNS had never worked on Windows, while logging that it had** — found in the same session's logs. `ApplyGlobal` drove its apply loop off the *backup* file, whose `InterfaceIndex` values go stale (wintun takes a new index on every agent start), so it configured adapters that no longer existed — `No MSFT_DNSClientServerAddress objects found with property 'InterfaceIndex' equal to '16'` — reached `applied=0`, and logged `system DNS now routed through the agent` regardless. Magic DNS and malicious-domain filtering were entirely inert on every Windows peer while the log said otherwise. It now applies to the adapters that are actually up, treats the backup as what it is (a record of what to restore, not a list of what to change), re-resolves adapters by alias on restore, treats a vanished adapter as ordinary rather than as a failure that pinned the backup file in place forever, and refuses to report a takeover that configured nothing. Verified live: `adapters=2`, with `cloud4.blinex` and `google.com` both resolving on a peer where neither ever had. (v0.21.3)
- [x] **Fixed: the agent took the host's DNS down with it on Linux kernel-TUN peers** — reported live: the machine had no working DNS at all while the agent ran, and recovered the moment it was stopped. `dnsconfig.Apply` pointed systemd-resolved at `127.0.0.1:53535` as a **per-link** DNS server on `blinex0`, plus `~.` to make that link the default route for every query. But systemd-resolved queries a link's DNS server on a socket bound to that link, so the query was emitted *into the tunnel* — caught with tcpdump: `blinex0 Out IP 100.64.0.9.54307 > 127.0.0.1.53535: UDP, length 40` — encrypted and lost, since 127.0.0.1 is not a mesh address. The resolver on loopback never saw a single packet (`0 packets captured` on `lo`), resolved retried and downgraded (`UDP+EDNS0 → UDP → TCP → UDP → ...`) until every lookup timed out, and because `~.` made this the host's *only* DNS route, all name resolution died — not just mesh names. It had gone unnoticed because querying the resolver directly (`dig @127.0.0.1 -p 53535`) always worked perfectly; only the path through the OS was broken, which also means Magic DNS had never actually resolved through the system resolver on these peers. The resolver now also binds the peer's own mesh IP and resolved is pointed there: that address has a `local … scope host` route via `blinex0`, so the kernel delivers it locally even on a link-bound socket (the same reason Tailscale answers on 100.100.100.100 rather than loopback). Loopback stays bound too, so hand-configured resolvers and the netstack `:53 → :53535` relay are unaffected. Hardened against the whole class: every listener is bound *before* the OS override goes in, and if the mesh listener can't bind, the override is skipped entirely and system DNS is left alone — Magic DNS degrades instead of taking the host's name resolution with it. That safety net proved itself during this very fix: the first live run passed a CIDR (`100.64.0.9/32:53535`) as a bind address, the bind failed, and the host kept resolving normally. Verified live on a Linux kernel-TUN peer: public DNS, the resolved stub, and `getent hosts virtwin.blinex` all resolve with the agent running; clean stop reverts; SIGKILL still self-heals via interface teardown. (v0.21.2)
- [x] **Fixed: direct paths that couldn't carry traffic were promoted anyway** — found live against a Windows peer behind a home NAT: the link oscillated relay→direct→relay every ~12 seconds indefinitely, black-holing traffic for ~9s of each cycle (33–100% ICMP loss to that peer, 17 flaps in five minutes, while a peer on a stable relay path showed 0%). The health test was the bug: `healthy` was `time.Since(lastPong) < probeTimeout`, so one stray pong satisfied *every* probe tick inside the 10s window — a single packet promoted a path that could not actually carry traffic, and once the window lapsed the path was re-promoted three seconds later, having just proven itself bad. There was neither hysteresis nor backoff. Compounding it, the probe was one byte, which never demonstrated the path could carry a full-size WireGuard packet — precisely the failure mode of a marginal NAT binding that passes tiny datagrams and drops real ones. A tick now counts as answered only if a *new* pong arrived since the previous tick; promotion requires three consecutive answered probes (~9s proven); a stall reverts to relay and holds it there for 15s, doubling to a 5min cap, forgiven after two minutes of stability. Probes are padded to 1200 bytes so a round trip proves what it claims to, and probe matching keys on the first byte at any length so peers on older builds sending 1-byte probes still interoperate. The policy lives in a timer-free `probeState.observe()` so it is unit-testable directly rather than only observable through a live link — the tests were confirmed to fail against the old health check, reproducing the live symptom exactly. Verified live: 0 flaps and 0% loss over 60 pings on the same link. The inverse case ("still upgrades a genuinely good path") rests on the unit tests — this topology has no working direct path to promote, since the peer's cloud firewall blocks inbound UDP. (v0.21.1)
- [x] **Windows ACL enforcement, subnet routing, and exit-node parity** — v0.20.0's real kernel interface flipped `NetstackMode()` to false, which was also what gated ACL enforcement, subnet-router forwarding, and exit-node activation. Those paths call Linux-only iptables/netlink code that simply errors out elsewhere, and the netstack userspace fallback no longer ran either. Net effect: a Windows peer in kernel mode had **zero ACL enforcement** — a real regression from when Windows was always netstack mode and did enforce it — and could neither provide nor consume subnet routes or exit nodes. Windows Firewall can't substitute for the iptables `BLINEX-ACL` chain: its block rules always win over allow rules regardless of scope or creation order, so "default deny, a few explicit allows" isn't expressible the way an ordered chain expresses it. Instead the filter sits where wireguard-go itself sits — `aclFilterTUN` wraps the real wintun device and evaluates policy on every decrypted packet passing through `Write()` into the OS (the same INPUT+FORWARD traffic `BLINEX-ACL` covers on Linux), in plain Go, so evaluation order and default-deny are what the code says rather than an approximation through OS primitives. The packet-matching logic mirrors the netstack router's semantics exactly and is kept platform-agnostic specifically so it has a real, executable Linux-side unit test rather than only compiling under `GOOS=windows` unverified. Routing uses `New-NetNat` for masquerade (Windows-native forwarding+NAT together, no `ip_forward` equivalent needed) and `New-NetRoute` for advertised routes, exit-node split-tunnel /1s, and management/signal host-route pinning. (v0.21.0)
- [x] **Fixed: a Windows peer could ping out but never be pinged** — verified live after v0.20.0 shipped kernel TUN: ICE's own connectivity checks showed the tunnel healthy in both directions the whole time, ruling out transport. A freshly created Windows adapter defaults to the "Public" firewall profile, which blocks unsolicited inbound traffic including ICMP echo — the pings were arriving and being silently dropped by the OS firewall. `setKernelAddress` now sets the adapter's connection profile to Private (the mesh is a trusted network — the same reasoning Tailscale and WireGuard-for-Windows apply) and adds one explicit, interface-scoped inbound-allow rule for ICMPv4 echo, so it holds even under a policy that overrides the profile default. (v0.20.1)
- [x] **Real kernel TUN on Windows, via wintun** — Windows agents had only ever run in userspace netstack mode: `createTUN` unconditionally failed on every non-Linux platform, so there was never a real OS network interface for the mesh. That is why plain ping — or any other OS-level traffic — to a peer's mesh IP had never worked from a Windows agent no matter how healthy ICE/TURN was underneath: nothing in the Windows routing table pointed at `100.64.0.0/10`. The agent now creates a real wintun adapter through wireguard-go's existing Windows support, embedding the official prebuilt `wintun.dll` per-arch and extracting it next to the running executable (Windows searches only the application directory and System32 for it, never `PATH`), then assigns the mesh IP and route via PowerShell with retries, since the adapter takes a moment to become visible to the rest of the networking stack. It falls back to the existing netstack path automatically if wintun creation fails for any reason — e.g. not running elevated — so it cannot regress an existing install. This also exposed a latent coupling: `engine.go` used `NetstackMode()` to choose between per-link and global DNS override, which held only while netstack == "not Linux" == "no per-link DNS support" were the same set. Windows has no per-link DNS implementation at all, kernel TUN or not, so that decision is now its own per-platform `UsesGlobalDNS()`. (v0.20.0)
- [x] **Fixed: three bugs behind an extended Windows connectivity incident** — (1) The Windows *service* path never configured a logger at all, so every log line went nowhere for a process with no attached console; wireguard-go's own internal logging was worse, writing straight to `os.Stdout` and bypassing zerolog entirely on *every* platform. The service now logs to `%ProgramData%\blinex\agent.log`, and wgmgr builds its own `*device.Logger` wired into zerolog — handshake attempts and failures are now actually visible, which they never were before. (2) `RevertGlobal` on Windows logged "restored original adapter DNS settings" and deleted the backup file unconditionally, even when the underlying `Set-DnsClientServerAddress` had failed — and Windows tears down the CIM/WMI provider those calls depend on before a service finishes shutdown cleanup, so a revert-on-reboot genuinely can fail. The false "success" then permanently discarded the one record of the real original DNS settings, leaving a device stuck on a `127.0.0.1` override with no way to self-heal. Revert now tracks per-adapter success and clears the backup only once every adapter actually reverted. A related deadlock is broken too: nothing previously restored a stale override *before* enrollment, and enrollment needs working DNS to resolve the management server, so a device stuck this way could never reach the code that would have recovered it — `RecoverStaleGlobalOverride` now runs once at startup, ahead of the first enrollment attempt. (3) The Windows `install` command's generated config omitted `stun_urls`/`turn_user`/`turn_pass` entirely, so every service-installed Windows agent silently fell back to Google's public STUN with zero TURN candidates — meaning a device behind a symmetric NAT had no fallback candidate type at all. `install` now accepts `-stun-url`/`-turn-user`/`-turn-pass` and warns clearly if they are omitted. (v0.19.2)
