...
### Next up
- [ ] **Proper Windows service install (with auto-restart)** — the Windows agent still runs as a foreground process a user starts by hand, so unlike Linux (`systemd Restart=on-failure`), a crash leaves DNS pointed at a dead resolver until someone notices and restarts it manually. Live-tested during the netstack DNS rollout: VirtWin needed a manual console restart after a process swap and stayed unreachable for several minutes in the meantime — exactly the risk this was flagged for in advance. Needs either a real Windows service wrapper or NSSM-style supervision before the DNS auto-configuration story is as safe on Windows as it already is on Linux
