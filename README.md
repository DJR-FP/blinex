...
- **Magic DNS, actually automatic** — on Linux kernel-TUN peers, the agent points the OS at its own resolver on startup (via `resolvectl`, like NetBird/Tailscale's MagicDNS) and cleanly reverts on exit, so hostnames and domain filtering both work with zero manual DNS configuration. Netstack peers (no real interface — Windows, macOS, unprivileged LXC) still need DNS pointed at `127.0.0.1:53535` by hand for now
...
### Next up
- [ ] **Automatic Magic DNS on netstack peers + Windows** — v0.17.0 covers Linux kernel-TUN only (a real interface to tie the DNS override's lifetime to); netstack peers (Windows, macOS, unprivileged LXC) have no such interface, so they need a different mechanism — likely a global resolver override with its own crash-recovery story, since there's no OS-level link teardown to lean on
...
### Done ✅
- [x] **Automatic Magic DNS (Linux kernel-TUN)** — the agent now points the OS at its own resolver on startup via `resolvectl` (`resolvectl dns blinex0 127.0.0.1:53535` + `resolvectl domain blinex0 ~.`, making the mesh interface the *default* DNS route, not a split-DNS exception) and reverts on clean shutdown. This is what actually makes "on by default" true for Magic DNS hostnames and malicious-domain filtering alike — previously both only worked if a user manually pointed their system DNS at `127.0.0.1:53535`, which essentially nobody would ever do, so domain filtering was silently inert for anyone who hadn't hand-configured DNS. Crash-safe by construction rather than by added recovery logic: a non-persistent kernel TUN device is destroyed by the OS the instant its owning process dies, and systemd-resolved drops the per-link DNS override the moment the link disappears — verified live with a SIGKILL, DNS kept working system-wide within about a second, no stuck resolver, no state file needed. Scoped to kernel-TUN only for now; netstack peers (no real interface for the override to be tied to) and Windows remain on manual configuration until a global-override mechanism is designed. `dnsconfig.Apply`/`Revert` in a new `client/internal/dnsconfig` package, gated on `!wg.NetstackMode()`. (v0.17.0)
