...
| `RELAY_REALM` | `meshnet.io` | TURN realm string |
| `RELAY_AUTH_USER` | `meshnet` | Long-term credential username |
| `RELAY_AUTH_PASS` | `change-me-in-production` | Long-term credential password |

## Agent configuration

To use this relay, set `MESHNET_STUN_URLS` on the agent:

```bash
MESHNET_STUN_URLS="stun:stun.l.google.com:19302,turn:relay.example.com:3478" \
  ./bin/agent
```
